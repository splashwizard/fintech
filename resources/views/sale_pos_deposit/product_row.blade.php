<tr class="product_row @if(isset($is_service) && $is_service == 1) service_row @endif" data-row_index="{{$row_count}}">
	<td>
		...
		<input type="hidden" class="account_id" name="products[{{$row_count}}][account_id]" value="{{$product->account_id}}">
		<input type="hidden" name="products[{{$row_count}}][no_bonus]" value="{{$product->no_bonus}}">
...
{{--			<br>--}}
	...
	<td class="text-center v-center">
		...
		@endphp
		<input type="{{$subtotal_type}}" name="products[{{$row_count}}][amount]" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="@if(empty($amount)){{$product->quantity_ordered*$unit_price_inc_tax }}@else{{$amount}}@endif">
