...
<section class="content">
  ...
  <div class="box box-solid">
    <div class="box-body">
      ...
      <input id="promotion_cnt" value="{{count($promotions)}}" hidden>
      <div id="exTab2">
          ...
          @foreach($promotions as $key => $promotion)
            @if($key == 0)
              <div class="tab-pane active" id="tab0">
                @include('promotion.form', ['form_index' => 0, 'promotion' => $promotion])
              </div>
            @else
              <div class="tab-pane" id="tab{{$key}}">
                @include('promotion.form', ['form_index' => $key, 'promotion' => $promotion])
              </div>
            @endif
          @endforeach
        </div>
...
@section('javascript')
  <script>
    ...
    $('#new_tab').click(function (e) {
      e.preventDefault();
      var promotion_cnt = $('#promotion_cnt').val();
      $.ajax({
        url: "/promotions/" + promotion_cnt + "/getTab",
        dataType: 'json',
        success: function(result) {
          $('.nav-tabs').append('<li>\n' +
                  '                <a href="#tab' + promotion_cnt +'" data-toggle="tab">New Tab</a>\n' +
                  '              </li>');
          $('.tab-content').append('<div class="tab-pane" id="tab' + promotion_cnt + '">' +
                  result.html +
                  '</div>');
          $('#promotion_cnt').val( promotion_cnt + 1);
          $('.start_time, .end_time').datetimepicker({format: 'YYYY-MM-DD hh:mm:ss'});
