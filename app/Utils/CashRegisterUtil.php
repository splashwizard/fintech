        ...
        if ($status_before == 'draft' && $transaction->status == 'final') {
            ...
            $prev_payments = CashRegisterTransaction::where('transaction_id', $transaction->id)
                            ->select(
                                ...
                                DB::raw("SUM(IF(pay_method='service_transfer', IF(type='credit', amount, -1 * amount), 0)) as total_service_transfer"),
                                DB::raw("SUM(IF(pay_method='bonus', IF(type='credit', amount, -1 * amount), 0)) as total_bonus"),
                                DB::raw("SUM(IF(pay_method='bonus', IF(type='credit', amount, -1 * amount), 0)) as total_bank_transfer"),
            ...
            if (!empty($prev_payments)) {
                $payment_diffs = [
                    ...
                    'service_transfer' => $prev_payments->total_service_transfer,
                    'bonus' => $prev_payments->total_bonus,
